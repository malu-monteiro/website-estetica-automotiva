...
export const ServicesCarousel: React.FC<ServicesCarouselProps> = ({
  ...
  return (
    <div className="relative w-full">
            ...
            <div key={index} className="flex-none w-80 md:w-96">
              ...
              >
                <div className="relative h-full z-10">
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-end bg-gradient-to-t from-zinc-900 via-zinc-900/40 to-transparent opacity-100 will-change-auto">
                    <h3 className="text-xl md:text-2xl font-bold uppercase mb-2 text-start">
                      {item.title}
                    </h3>
                    <p className="text-sm text-gray-300 mb-4 line-clamp-2 text-start">
                      {item.description}
                    </p>
                    <div className="flex items-center font-semibold uppercase text-sm group-hover:text-red-800 transition-colors">
      ...
      {/* Navigation Buttons */}
      <button
        className="absolute top-1/2 left-4 -translate-y-1/2 p-2 bg-gray-800/80 rounded-full hover:bg-gray-700 transition-colors backdrop-blur-sm"
        onClick={scrollPrev}
      ...
      <button
        className="absolute top-1/2 right-4 -translate-y-1/2 p-2 bg-gray-800/80 rounded-full hover:bg-gray-700 transition-colors backdrop-blur-sm"
        onClick={scrollNext}
